...
dat.summplot.l <- lapply(dat.summ.l, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein', 'Peptide'), 'mean')) # Raw (you still need to aggregate the data because of 'dat.summ.l <- dat.unit.l' operation)
dat.normplot.l <- lapply(dat.norm.l, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein', 'Peptide'), 'mean')) # Normalized
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1, 2))
    pcaplot.ils(dat.summplot.w[[i]], run.labels, conditions.vec, cols.vec, paste('Raw', variant.names[i], sep='_'), scale=T)
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1, 2))
    dendrogram.ils(dat.summplot.w[[i]], sample.labels, cols.vec, paste('Raw', variant.names[i], sep='_'))
