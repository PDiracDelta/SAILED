...
dat.unit.l[[3]] <- dat.l %>% left_join(denom.df, by=c('Run', 'Protein', 'Peptide', 'RT', 'Charge', 'PTM')) %>% mutate(response=log2(Intensity/denom)) %>% select(-c(Intensity))
```
...
dat.summplot.l <- lapply(dat.summ.l, function(x) aggFunc(x, 'response', 'mean')) # Raw (you still need to aggregate the data because of 'dat.summ.l <- dat.unit.l' operation)
dat.normplot.l <- lapply(dat.norm.l, function(x) aggFunc(x, 'response', 'mean')) # Normalized
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1,2))
    boxplot.ils(dat.summ.l[[i]], paste('Raw', variant.names[i], sep='_'))
    boxplot.ils(dat.norm.l[[i]], paste('Normalized', variant.names[i], sep='_'))
  par(mfrow=c(1,1))
}
```
...
for (i in 1: n.comp.variants){
  p1 <- maplot.ils(dat.summplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale.vec[i], paste('Raw', variant.names[i], sep='_'))
  
  p2 <- maplot.ils(dat.normplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale.vec[i], paste('Normalized', variant.names[i], sep='_'))
...
for (i in 1: n.comp.variants){
  p1 <- maplot.ils(dat.summplot.w[[i]], samples.num, samples.denom, scale=scale.vec[i], paste('Raw', variant.names[i], sep='_'))
  
  p2 <- maplot.ils(dat.normplot.w[[i]], samples.num, samples.denom, scale=scale.vec[i], paste('Normalized', variant.names[i], sep='_'))
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1, 2))
    cvplot.ils(dat=dat.summplot.l[[i]], feature.group='Peptide', xaxis.group='Condition', 
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1, 2))
    pcaplot.ils(dat.summplot.w[[i]][spiked.peptides, ], run.labels, conditions.vec, cols.vec, paste('Raw', variant.names[i], sep='_'), scale=T)
    pcaplot.ils(dat.normplot.w[[i]][spiked.peptides, ], run.labels, conditions.vec, cols.vec, paste('Normalized', variant.names[i], sep='_'))
  par(mfrow=c(1, 1))  
}
```
...
for (i in 1: n.comp.variants){
  par(mfrow=c(1, 2))
    dendrogram.ils(dat.summplot.w[[i]][spiked.peptides, ], sample.labels, cols.vec, paste('Raw', variant.names[i], sep='_'))
    dendrogram.ils(dat.normplot.w[[i]][spiked.peptides, ], sample.labels, cols.vec, paste('Normalized', variant.names[i], sep='_'))
  par(mfrow=c(1, 1))  
}
```
