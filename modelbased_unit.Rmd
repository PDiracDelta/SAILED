...
```{r}
# the names are alredy present in the list object. Use them instead of indices.
...
```{r}
# copy paste the new log2 ratio approach from my notebooks
...
dat.summplot.w <- lapply(dat.summplot.l, function(x) {
  return( pivot_wider(data=x, id_cols=Peptide, names_from=Run:Channel, values_from=response, names_sep=':') %>% column_to_rownames('Peptide') ) } )
...
for (i in 1: n.comp.variants){
  ...
  cvplot.ils(dat=dat.summplot.l[[i]], feature.group='Peptide', xaxis.group='Condition', 
             title=paste('Before normalization', variant.names[i], sep='_'))
...
for (i in 1: n.comp.variants){
  pcaplot.ils(dat.summplot.w[[i]][spiked.peptides, ], run.labels, conditions.vec, cols.vec, paste('Before normalization', variant.names[i], sep='_'), scale=T)
  pcaplot.ils(dat.normplot.w[[i]][spiked.peptides, ], run.labels, conditions.vec, cols.vec, paste('After normalization', variant.names[i], sep='_')) }
```
...
for (i in 1: n.comp.variants){
  dendrogram.ils(dat.summplot.w[[i]][spiked.peptides, ], sample.labels, cols.vec, paste('Before normalization', variant.names[i], sep='_'))
  dendrogram.ils(dat.normplot.w[[i]][spiked.peptides, ], sample.labels, cols.vec, paste('After normalization', variant.names[i], sep='_'))
  par(mfrow=c(1, 1))  
}
```
