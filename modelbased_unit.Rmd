...
```{r}
# group by run and spectrum ID (RT), take mean of all intensities (channelNames) then ungroup runs again.
...
# dat.unit.l[[3]] <- dat.l %>% left_join(denom.df, by=c('Run', 'Protein', 'Peptide', 'RT', 'Charge', 'PTM')) %>% mutate(response=Intensity/denom) %>% select(-c(Intensity, denom)) #

denom.df=dat.l %>% pivot_wider(id_cols=-one_of('Condition', 'BioReplicate'),names_from='Channel', values_from='Intensity')
# a=denom.df %>% pivot_longer(cols=all_of(paste0(rep(127:130, each=2), c('C','N'))),names_to='Channel', values_to="Intensity")
denom.df$denom=apply(denom.df[,paste0(rep(127:130, each=2), c('C','N'))], 1, mean)
denom.df=denom.df[,c('Run', 'Protein', 'Peptide', 'RT', 'Charge', 'PTM', 'denom')]
#denom.df %>% group_by(Run, Protein, Peptide, RT, Charge, PTM, denom) %>% summarise(nn=n()) %>% filter(nn>1)
...
# a=dat.unit.l[[3]]
# b=dat.unit.l[[5]]
# x=dat.unit.l[[3]]$response
# ind=!is.na(x)
# x=x[ind]
# y=dat.unit.l[[5]]$response[ind]
# head(x)
# head(y)
```
...
for (i in 1: n.comp.variants){
  p1 <- maplot_ils(dat.summplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale.vec[i], paste('Before normalization', variant.names[i], sep='_'))
  p2 <- maplot_ils(dat.normplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale.vec[i], paste('After normalization', variant.names[i], sep='_'))
...
for (i in 1: n.comp.variants){
  p1 <- maplot_ils(dat.summplot.w[[i]], samples.num, samples.denom, scale=scale.vec[i], paste('Before normalization', variant.names[i], sep='_'))
  p2 <- maplot_ils(dat.normplot.w[[i]], samples.num, samples.denom, scale=scale.vec[i], paste('After normalization', variant.names[i], sep='_'))
  grid.arrange(p1, p2, ncol=2)}
```
...
par(mfrow=c(1,2))
for (i in 1: n.comp.variants){
  dendrogram_ils(dat.summplot.w[[i]][rownames(dat.summplot.w[[i]]) %in% spiked.proteins,], info=sample.info, paste('Before normalization', variant.names[i], sep='_'))
