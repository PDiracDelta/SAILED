...
p[[1]] <- maplot_ils(dat.summplot.w, 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', 'Before normalization')
for (i in 1: n.comp.variants){
  p[[i+1]] <- maplot_ils(dat.normplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', paste('After normalization', variant.names[i], sep='_'))}
...
p <- emptyList(c('raw', names(dat.normplot.w)))
p[[1]] <- maplot_ils(dat.summplot.w, samples.num, samples.denom, scale='log', 'Before normalization')
for (i in 1: n.comp.variants){
  p[[i+1]] <- maplot_ils(dat.normplot.w[[i]], samples.num, samples.denom, scale='log', paste('After normalization', variant.names[i], sep='_')) }
grid.arrange(grobs = p, ncol=2, nrow=2)
```
...
for (i in 1: n.comp.variants){
  cvplot_ils(dat=dat.normplot.l[[i]], feature.group='Protein', xaxis.group='Condition', title=paste('After normalization', variant.names[i], sep='_'))}
par(mfrow=c(1,1))
```
...
for (i in 1: n.comp.variants){
  pcaplot_ils(dat.normplot.w[[i]], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
for (i in 1: n.comp.variants){
  pcaplot_ils(dat.normplot.w[[i]][rownames(dat.normplot.w[[i]]) %in% spiked.proteins, ], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
dat.norm.l.spk <- lapply(dat.norm.l.spk, function(x) x[[1]] <- dat.summ.l %>% filter(Protein %in% spiked.proteins))
LMM1 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Protein)  + (1|Mixture:TechRepMixture:Peptide), data=dat.summ.l %>% filter(Protein %in% spiked.proteins))
dat.norm.l.spk[[1]]$response <- residuals(LMM1)

LMM2 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Mixture:TechRepMixture:Protein)  + (1|Peptide), data=dat.summ.l %>% filter(Protein %in% spiked.proteins))
dat.norm.l.spk[[2]]$response <- residuals(LMM2)

LMM3 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Protein)  + (1|Peptide), data=dat.summ.l %>% filter(Protein %in% spiked.proteins))
dat.norm.l.spk[[3]]$response <- residuals(LMM3)

dat.normplot.spk.l <- lapply(dat.norm.l.spk, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein'), 'mean')) 
...
pcaplot_ils(dat.summplot.w[rownames(dat.summplot.w) %in% spiked.proteins, ], info=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
  pcaplot_ils(dat.normplot.spk.w[[i]], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1,1))
```
...
dat.summ.l <- dat.summ.l %>% mutate(protein.type=ifelse(Protein %in% spiked.proteins, 'spiked', 'background'))
wgts <- dat.summ.l %>% group_by(protein.type) %>% summarise(nn=n_distinct(Protein))
...
dat.summ.l$weights <- ifelse(dat.summ.l$protein.type=='spiked', wgt.spiked, wgt.background)

dat.norm.l.wgt <- emptyList(variant.names)
dat.norm.l.wgt <- lapply(dat.norm.l, function(x) x[[1]] <- dat.summ.l)
LMM1 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Protein)  + (1|Mixture:TechRepMixture:Peptide), data=dat.summ.l, weights=weights)
dat.norm.l.wgt[[1]]$response <- residuals(LMM1)
LMM2 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Mixture:TechRepMixture:Protein)  + (1|Peptide), data=dat.summ.l, weights=weights)
dat.norm.l.wgt[[2]]$response <- residuals(LMM2)
LMM3 <- lmer(response ~ Mixture + Mixture:TechRepMixture + Mixture:TechRepMixture:Channel + (1|Protein)  + (1|Peptide), data=dat.summ.l, weights=weights)
dat.norm.l.wgt[[3]]$response <- residuals(LMM3)

dat.normplot.wgt.l <- lapply(dat.norm.l.wgt, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein'), 'mean')) 
...
pcaplot_ils(dat.summplot.w[rownames(dat.summplot.w) %in% spiked.proteins,], info=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
    pcaplot_ils(dat.normplot.wgt.w[[i]][rownames(dat.normplot.wgt.w[[i]]) %in% spiked.proteins, ], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
for (i in 1: n.comp.variants){
  dendrogram_ils(dat.normplot.w[[i]][rownames(dat.normplot.w[[i]]) %in% spiked.proteins,], info=sample.info, paste('After normalization', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
dat.dea <- lapply(dat.norm.l, function(x){
  return(lmm_dea(dat=x, mod.formula='response ~ Condition + (1|Run:Channel)', referenceCondition))})
