...
```{r}
par(mfrow=c(2,2))
boxplot.ils(dat.summ.l, 'Before normalization')
...
p[[1]] <- maplot.ils(dat.summplot.w, 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', 'Before normalization')
for (i in 1: n.comp.variants){
  p[[i+1]] <- maplot.ils(dat.normplot.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', paste('After normalization', variant.names[i], sep='_'))}
...
p <- emptyList(c('raw', names(dat.normplot.w)))
p[[1]] <- maplot.ils(dat.summplot.w, samples.num, samples.denom, scale='log', 'Before normalization')
for (i in 1: n.comp.variants){
  p[[i+1]] <- maplot.ils(dat.normplot.w[[i]], samples.num, samples.denom, scale='log', paste('After normalization', variant.names[i], sep='_')) }
grid.arrange(grobs = p, ncol=2, nrow=2)
```
...
```{r}
par(mfrow=c(2,2))
cvplot.ils(dat=dat.summplot.l, feature.group='Peptide', xaxis.group='Condition', 
...
```{r}
par(mfrow=c(2, 2))
pcaplot.ils(dat.summplot.w[[1]], sample=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
  pcaplot.ils(dat.normplot.w[[i]], sample=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
par(mfrow=c(2, 2))
  pcaplot.ils(dat.summplot.w[[1]][rownames(dat.summplot.w[[1]]) %in% spiked.peptides, ], sample=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
  pcaplot.ils(dat.normplot.w[[i]][rownames(dat.summplot.w[[1]]) %in% spiked.peptides, ], sample=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
pcaplot.ils(dat.summplot.w[[1]][rownames(dat.summplot.w[[1]]) %in% spiked.peptides, ], sample=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
    pcaplot.ils(dat.normplot.wgt.w[[i]][rownames(dat.normplot.wgt.w[[1]]) %in% spiked.peptides, ], sample=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
```{r}
par(mfrow=c(2,2))
dendrogram.ils(dat.summplot.w, info=sample.info, 'Before normalization')
for (i in 1: n.comp.variants){
  dendrogram.ils(dat.normplot.w[[i]], info=sample.info, paste('After normalization', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
