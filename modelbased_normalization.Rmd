...
dat.summplot.l <- aggFunc(dat.summ.l, 'response', 'mean') # before normalization (you still need to aggregate the data because of 'dat.summ.l <- dat.unit.l' operation)
...
dat.summplot.w <- pivot_wider(data=dat.summplot.l, id_cols=Peptide, names_from=Run:Channel, values_from=response, names_sep=':') %>% column_to_rownames('Peptide')
...
```{r}
par(mfrow=c(2,2))
cvplot_ils(dat=dat.summplot.l, feature.group='Peptide', xaxis.group='Condition', title='Before normalization')
for (i in 1: n.comp.variants){
  cvplot_ils(dat=dat.normplot.l[[i]], feature.group='Peptide', xaxis.group='Condition', title=paste('After normalization', variant.names[i], sep='_'))}
par(mfrow=c(1,1))
```
...
par(mfrow=c(2, 2))
pcaplot_ils(dat.summplot.w[rownames(dat.summplot.w) %in% spiked.peptides, ], info=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
  pcaplot_ils(dat.normplot.w[[i]][rownames(dat.summplot.w) %in% spiked.peptides, ], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
...
pcaplot_ils(dat.summplot.w[rownames(dat.summplot.w) %in% spiked.peptides, ], info=sample.info, 'Raw', scale=T)
for (i in 1: n.comp.variants){
    pcaplot_ils(dat.normplot.wgt.w[[i]][rownames(dat.normplot.wgt.w[[1]]) %in% spiked.peptides, ], info=sample.info, paste('Normalized', variant.names[i], sep='_'))}
par(mfrow=c(1, 1))
```
