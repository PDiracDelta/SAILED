...
dat.norm.summ.w2 <- dat.norm.w %>% group_by(Run, Protein, Peptide) %>% summarise_at(.vars = quanCols, .funs = median) %>% summarise_at(.vars = quanCols, .funs = median) %>% ungroup()

# make data completely wide (also across runs)
dat.norm.summ.w22 <- dat.norm.summ.w2 %>% pivot_wider(names_from = Run, values_from =all_of(quanCols), names_glue = "{Run}:{.value}")

design.matrix <- get_design_matrix(referenceCondition, sample.info)
d <- column_to_rownames(as.data.frame(dat.norm.summ.w22), 'Protein')
