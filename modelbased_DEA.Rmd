...
## mixed model (intra-protein correlation) + eBayes (PSM level data)

```{r}
dat.dea$LMM <- lmm_dea(dat=dat.norm.l, mod.formula='response ~ Condition + (1|Run:Channel)', referenceCondition) 
...
q.cols <- dea.cols[stri_detect_fixed(dea.cols, 'p.mod')]
