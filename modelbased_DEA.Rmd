...
load(paste0('modelbased_DEA_outdata_', params$suffix_p, '.rda'))
```
...
```{r}
p1 <- maplot_ils(dat.nonnorm.summ.w2, ma.onesample.num, 'Mixture1_2:129N', scale='log', 'raw', spiked.proteins)
p2 <- maplot_ils(dat.norm.summ.w2, ma.onesample.num, 'Mixture1_2:129N', scale='log', 'normalized', spiked.proteins)
...
```{r}
channels.num <- sample.info %>% filter(Condition==maplot.allsamples.num) %>% distinct(Sample) %>% pull
channels.denom <- sample.info %>% filter(Condition==maplot.allsamples.denom) %>% distinct(Sample) %>% pull
