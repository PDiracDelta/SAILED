...
dat.nonnorm.summ.l <- lapply(dat.summ.l, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein'), 'median'))

# after normalization
dat.norm.summ.l <- lapply(dat.norm.l, function(x) aggFunc(x, 'response', group.vars=c('Mixture', 'TechRepMixture', 'Run', 'Channel', 'Condition', 'BioReplicate', 'Protein'), 'median'))
...
for (i in 1: n.comp.variants){
  p1 <- maplot_ils(dat.nonnorm.summ.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', paste('Before normalization', variant.names[i], sep='_'), spiked.proteins)
  p2 <- maplot_ils(dat.norm.summ.w[[i]], 'Mixture2_1:127C', 'Mixture1_2:129N', scale='log', paste('After normalization', variant.names[i], sep='_'), spiked.proteins)
...
for (i in 1: n.comp.variants){
  p1 <- maplot_ils(dat.nonnorm.summ.w[[i]], samples.num, samples.denom, scale='log', paste('Before normalization', variant.names[i], sep='_'), spiked.proteins)
  p2 <- maplot_ils(dat.norm.summ.w[[i]], samples.num, samples.denom, scale='log', paste('After normalization', variant.names[i], sep='_'), spiked.proteins)
  grid.arrange(p1, p2, ncol=2)}
```
...
par(mfrow=c(1, 2))
for (i in 1: n.comp.variants){
  cvplot_ils(dat=dat.nonnorm.summ.l[[i]], feature.group='Protein', xaxis.group='Condition', title=paste('Before normalization', variant.names[i], sep='_'), abs=F)
  cvplot_ils(dat=dat.norm.summ.l[[i]], feature.group='Protein', xaxis.group='Condition', title=paste('After normalization', variant.names[i], sep='_'), abs=F)}
par(mfrow=c(1, 1))
```
