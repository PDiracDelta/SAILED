...
```{r}
dat.summ.l$median <- aggFunc(dat.unit.l[[1]], 'response', 'median')
```

## PSM to peptide summarization using sum

```{r}
dat.summ.l$sum <- aggFunc(dat.unit.l[[1]], 'response', 'sum')
```
...
dat.dea <- lapply(dat.norm.l, function(x){
  ...
  return( mixed.model.dea(dat=tmp[1:1000,], mod.formula='response ~ Condition + (1|Run:Channel)', conditions=c('1', '0.125')) ) } ) # subsetting to first 1000 records to speed up knitting
```
