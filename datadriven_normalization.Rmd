...
```{r, echo=FALSE}
if (!load_outputdata){
# propagate the second round of median sweeping and quantile normalization from dat.norm.summ.w2 to dat.norm.summ.w
  dat.norm.summ.w$medianSweeping <- dat.norm.summ.w2$medianSweeping %>% pivot_longer(cols=-one_of('Protein'), names_to='Sample', values_to='response', values_drop_na = T) %>% inner_join(sample.info[,c('Sample', 'Run', 'Channel', 'Condition')], by='Sample') %>% pivot_wider(id_cols=-one_of('Condition', 'Sample'), names_from=Channel, values_from=response)
  
  dat.norm.summ.w$quantile <- dat.norm.summ.w2$quantile %>% pivot_longer(cols=-one_of('Protein'), names_to='Sample', values_to='response', values_drop_na = T) %>% inner_join(sample.info[,c('Sample', 'Run', 'Channel', 'Condition')], by='Sample') %>% pivot_wider(id_cols=-one_of('Condition', 'Sample'), names_from=Channel, values_from=response)
}
```
...
dat.norm.summ.w$quantileByCondition <- pivot_wider(data = x, id_cols=-one_of('Condition', 'Sample'), names_from=Channel, values_from=response)
dat.norm.summ.w2$quantileByCondition <- dat.norm.summ.w$quantileByCondition %>% pivot_wider(names_from = Run, values_from = all_of(channelNames), names_glue = "{Run}:{.value}")
}
variant.names <- c('medianSweeping', 'CONSTANd', 'NOMAD', 'quantile', 'quantileByCondition')
n.comp.variants <- length(variant.names)
```
