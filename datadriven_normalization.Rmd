...
p[[1]] <- maplot.ils(dat.nonnorm.summ.w2, '127C_Mixture2_1', '129N_Mixture1_2', scale.vec[i], 'Before normalization')

for (i in 1: n.comp.variants){
 p[[i+1]]<- maplot.ils(dat.norm.summ.w2[[i]], '127C_Mixture2_1', '129N_Mixture1_2', scale.vec[i], paste('After normalization', variant.names[i], sep='_'))
}
...
p <- vector('list', n.comp.variants+1)
p[[1]] <- maplot.ils(dat.nonnorm.summ.w2, channels.num, channels.denom, scale.vec[i], 'Before normalization')

for (i in 1: n.comp.variants){
 p[[i+1]]<- maplot.ils(dat.norm.summ.w2[[i]], channels.num, channels.denom, scale.vec[i], paste('After normalization', variant.names[i], sep='_'))
}
grid.arrange(grobs = p, ncol=2, nrow=2)  
```
...
par(mfrow=c(2, 2))
  pcaplot.ils(dat.nonnorm.summ.w2 %>% select(-'Protein'), run.labels, conditions.vec, cols.vec, 'Before normalization')
...
run.labels <- stri_replace(unlist(lapply(stri_split(colnames(dat.norm.summ.w2[[1]])[-1], fixed='_'), function(x) paste(x[2],x[3],sep = '_'))), fixed='Mixture', 'Mix')
par(mfrow=c(2, 2))
  pcaplot.ils(dat.nonnorm.summ.w2 %>% filter(Protein %in% spiked.proteins) %>% select(-'Protein'), run.labels, conditions.vec, cols.vec, 'Before normalization')
...
par(mfrow=c(2, 2))
  dendrogram.ils(dat.nonnorm.summ.w2 %>% filter(Protein %in% spiked.proteins) %>% select(-Protein), sample.labels, cols.vec, 'Before normalization')

for (i in seq_along(dat.norm.summ.w2)){
    dendrogram.ils(dat.norm.summ.w2[[i]] %>% filter(Protein %in% spiked.proteins) %>% select(-Protein), sample.labels, cols.vec, paste('After normalization', variant.names[i], sep='_'))
}
par(mfrow=c(1, 1))  
```
...
# scatterplot.ils(dat.dea, p.cols, 'p-values') # commented due to error, sd=0 for NOMAD and RAW
