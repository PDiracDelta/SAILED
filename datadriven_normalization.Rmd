...
```{r}
dat.tmp <- dat.unit.w %>% pivot_wider(names_from = Run, values_from = all_of(quanCols), names_glue = '{Run}:{.value}')
quanColsLogic <- colnames(dat.tmp) %in% sample.info$Sample
dat.tmp[,quanColsLogic] <- normalizeCyclicLoess(dat.tmp[,quanColsLogic])
# VSN is generalized log transform, so the method has to be applied on raw data
#library(vsn)
#normalizeVSN(dat.tmp[,quanColsLogic])
# switch from wide format to (half-)wide format data (with an intermediate step-switching to long format)
dat.norm.w$CycLoessVSN <- dat.tmp %>% pivot_longer(cols=colnames(dat.tmp)[quanColsLogic], names_to='Sample', values_to='response') %>% left_join(sample.info[,c('Run','Condition','Channel','Sample')], by='Sample') %>% drop_na(response) %>% 
  pivot_wider(id_cols=-one_of(c('Condition', 'Sample')), names_from=Channel, values_from=response)
```
