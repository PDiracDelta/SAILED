...
The Wilcoxon rank-sum test ([Wilcoxon, F.](http://doi.org/10.1007/978-1-4612-4380-9)), for brevity referred to as 'Wilcoxon' test, is a non-parametric rank-based test for comparing independent observations from two groups (i.e., biological conditions).
For each protein separately, the test is applied to each condition w.r.t. the reference condition `r referenceCondition`. 

Note that the Wilcoxon test employs ranks of quantification values, so fold changes are not formally provided by this testing procedure. However, our wrapper function `Wilcoxon_test` provides with two ways of (artificially) computing fold changes via `logFC.method` argument:

*  `'diff'`: $\bar{x}-\bar{y}$ 
*  `'ratio'`: $log_2(\frac{\bar{x}}{\bar{y}})$

where $\bar{}$ denotes the arithmetic mean operator, x and y are quantification values from samples belonging to a selected biological condition and the reference condition, respectively. The assumption is that the quantification values are on the log scale, so in the case of `'ratio'`, we first exponentiate them with base 2 to then calculate the arithmetic means. In this notebook we present the `'ratio'` protein fold changes.
...
On the other hand, the fold changes are very well correlated for all component variants, across all conditions, though the Wilcoxon estimates to a slightly lesser extent. Actually, the estimates corresponding to moderated t-test, permutation test, and ROTS are identical as all these methods use the difference between group averages in their test statistics.
