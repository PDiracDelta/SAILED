...
quanCols = setdiff(colnames(dat.unit.w), colnames(dat.unit.l))
dat.split <- split(dat.unit.w, dat.unit.w$Run)  # apply CONSTANd to each Run separately
dat.split.norm  <- lapply(dat.split, function(x) {
    x[,quanCols] <- CONSTANd(x[,quanCols])$normalized_data
    return(x) } )
dat.norm.w <- bind_rows(dat.split.norm)
```
...
## non-normalized data
dat.nonnorm.summ.w2 <- dat.nonnorm.summ.w %>% pivot_wider(names_from = Run, values_from = all_of(quanCols), names_sep = ":")
dat.nonnorm.summ.w2 <- flip_colnames(dat.nonnorm.summ.w2, 'Protein')

## normalized data
dat.norm.summ.w2 <- dat.norm.summ.w %>% pivot_wider(names_from = Run, values_from = all_of(quanCols), names_sep = ":")
dat.norm.summ.w2 <- flip_colnames(dat.norm.summ.w2, 'Protein')
```
...
par(mfrow=c(1,2))
  ...
  boxplot_w(dat.norm.summ.w, study.design, 'Normalized')
par(mfrow=c(1,1))
```
...
dat.nonnorm.summ.l <- to_long_format(dat.nonnorm.summ.w, study.design)

dat.norm.summ.w$Mixture <- unlist(lapply(stri_split(dat.norm.summ.w$Run,fixed='_'), function(x) x[1]))
dat.norm.summ.l <- to_long_format(dat.norm.summ.w, study.design)
...
dat.dea$Wilcoxon <- wilcoxon_test(dat.norm.summ.w2, referenceCondition, otherConditions, logFC.method='ratio')
```

## Permutation test

```{r}
dat.dea$permutation_test<- permutation_test(dat.norm.summ.w2, referenceCondition, otherConditions, B=1000, seed=seed)
```
