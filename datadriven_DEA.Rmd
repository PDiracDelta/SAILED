...
medianlineplot_ils(dat.nonnorm.summ.l, 'Before normalization')
medianlineplot_ils(dat.norm.summ.l, 'After normalization')
```
...
dat.norm.summ.w2 <- dat.norm.summ.w2 %>% select(-one_of(chr))
...
```{r}
otherConditions <- sample.info %>% distinct(Condition) %>% pull(Condition) %>% as.character %>% sort
